...
import { Search, Calendar, Clock, Plus, X, Edit, Trash2, BarChart3, Zap } from 'lucide-react';
...
const TournamentScheduleList = ({ matches = [], eventId, bracketId, onRefresh, onViewStats, isStaffView, onInputStats }) => {
  ...
  return (
    <div style={{ background: '#0f172a', minHeight: '100vh', padding: '0' }}>
      <div style={{ background: '#1a2332', padding: '24px', borderRadius: '12px', marginBottom: '30px', border: '1px solid #2d3748' }}>
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {!isRoundRobin && (
            <button onClick={() => setShowTBDMatches(!showTBDMatches)} style={{ padding: '12px 16px', border: '1px solid #2d3748', borderRadius: '8px', fontSize: '14px', backgroundColor: showTBDMatches ? '#3b82f6' : '#0f172a', color: '#e2e8f0', cursor: 'pointer', fontWeight: '600', transition: 'all 0.2s ease', whiteSpace: 'nowrap' }}>
              {showTBDMatches ? '✓ Show TBD' : 'Hide TBD'}
            </button>
          )}
          {canBulkSchedule && !isStaffView && (
            <button onClick={() => setShowBulkScheduleModal(true)} style={{ padding: '12px 20px', border: 'none', borderRadius: '8px', fontSize: '14px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', cursor: 'pointer', fontWeight: '600', transition: 'all 0.2s ease', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(102, 126, 234, 0.4)' }}>
